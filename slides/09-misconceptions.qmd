...
data <- mutate(data, 
               people = rep(c(500, 800, 500), c(21, 3, 3)) + sample((-10:10), size = 27, replace = T)*c(rep(1, 21), rep(3, 3), rep(1, 3)))
...
p1 <- ggplot(df) + geom_mosaic(aes(x = product(Treatment), conds = product(Stone_Size), fill = outcome)) + 
  ggmosaic::scale_x_productlist("Stone Size (Outcome)", labels = c("Large (F)", "Large (S)", "Small (F)", "Small (S)")) + 
  guides(fill = "none") + 
...
- [It's fine to pick the model you want to "win"](https://messages.google.com/web/conversations/Cgg4LTRFVR4zQRIBMw) and choose other models around it
