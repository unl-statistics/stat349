...
officehours: "Schedule an appointment at [https://calendly.com/vanderplas/office-hours](https://calendly.com/vanderplas/office-hours)"
office: Hardin 343D
classroom: Hardin ANSC A228-East
classhours: MW 10:00 - 11:15
final: May 8, 7:30-9:30 AM (Presentations, only if necessary)
---
...
+---------------------------------+-------------+-------------------------------+
| Assignments                     | Weight      | Notes                         |
+=================================+=============+===============================+
| Reading Quizzes & Participation | 10%         | Reading links on Canvas       |
|                                 |             |                               |
|                                 |             | Quiz due before class         |
+---------------------------------+-------------+-------------------------------+
| Homework                        | 50%         | Weekly(ish)                   |
|                                 |             |                               |
|                                 |             | \~1 week to work on it        |
|                                 |             |                               |
|                                 |             | Due at 6pm on Fridays         |
+---------------------------------+-------------+-------------------------------+
...
-   Come to class, unless you're sick (see @sec-illness).
    You get one excuse-free absence.

-   Keep up with the work as assigned, even if nothing is turned in.
    It is important to allow time for revision and for your thinking to mature.

-   Up to two homework assignments may be resubmitted for regrading (@sec-resubmission).

-   Communicate early and often.
    Especially if you're having trouble with something.
    See @sec-tech-issues for how to include the right details when asking for help.
...
Other useful resources have been added to the course canvas page.
All books and resources used in this course are available through the UNL library free of charge.
...
```{r schedule, eval = T, echo = F, fig.pos = 'h'}
  schedule |>
  bind_rows(tibble(Week = 17, Topic = "Finals Week")) |>
    mutate(weeksort = Week) |>
    mutate(weeksort = if_else(Topic=="Spring Break", 9.5, weeksort)) |>
    mutate(Week = if_else(weeksort > 9.5, Week-1, Week)) |>
    mutate(Week = if_else(Topic == "Spring Break", NA, Week)) |>
  mutate(Topic = if_else(Week < 16 & Topic != "Spring Break", sprintf("<a href='https://unl-statistics.github.io/stat349/weeks/week-%02d.html'> %s </a>", Week, Topic), Topic)) |>
  group_by(weeksort, Week, Topic) |>
  summarize(`Important Dates` = paste(unique(`Important Dates`), collapse = "<br>")) |>
  ungroup() %>%   
  mutate(`Important Dates` = if_else(`Important Dates` == "NA", NA_character_, `Important Dates`)) |>
  arrange(weeksort) |>
  select(-weeksort) |>
...
These quizzes will be due before the first class of the week (so, at 10am on Mondays).
...
You may submit a homework assignment that was not submitted in time as a resubmission within one week of the assignment due date, assuming it meets the conditions below.
...
I am happy to provide feedback on revisions (after submission) without a grade update, as long as I have time.
...
-   screenshots (not cell phone pictures),
-   text copied (CTRL-C) error messages, and
-   a description of what you have tried, and
-   a description of what you think should happen

If you have problems with your computer, operating system, etc., please consult ITS/HelpDesk first.
...
-   SSD: 117 Louise Pound Hall, 402-472-3787

-   CAPS: 402-472-4750

-   Emergency Alerts: https://unlalert.unl.edu

-   Campus Police: 402-472-2222

-   Title IX and Institutional Equity and Compliance: 128 Canfield Administration, titleixcoordinator\@unl.edu, 402-472-3417
